      ...
      <div className="w-full max-w-2xl bg-white h-full overflow-y-auto flex flex-col shadow-2xl">
        ...
        <div className="px-6 py-5 border-b border-gray-100 flex items-start justify-between sticky top-0 bg-white z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-full text-xl transition-colors">×</button>
          </div>
        </div>
        ...
        <div className="flex-1 p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-xl border border-gray-200 p-4 space-y-2" style={{ background: "#F4F5F7" }}>
              ...
              {c.current_location && (
                <p className="text-sm text-gray-600">📍 {c.current_location}{c.preferred_location ? ` → ${c.preferred_location}` : ""}</p>
              )}
              {c.wfh_preference && <p className="text-sm text-gray-600">🏠 {c.wfh_preference}</p>}
            </div>
          ...
          {skills.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {skills.map((s, i) => (
                  <span key={i} className="px-2.5 py-1 text-xs rounded-full bg-gray-100 text-gray-700 border border-gray-200">{s}</span>
                ))}
              </div>
            </div>
          )}
